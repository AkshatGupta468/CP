...
class segmenttree
{
    ...
    void rangeupdate(int a,int b,obj val){
        a--;b--;
        recrangeupdate(a,b,0,n-1,1,val);
    }
    void recrangeupdate(int a,int b,int l,int r,int index,obj val)
    {
        if(b<l||r<a)
            return ;
        if(a>=l && r<=b)
        ...
        int mid=(l+r)/2;
        recrangeupdate(a,b,l,mid,2*index,val);
        recrangeupdate(a,b,mid+1,r,2*index+1,val);
    }
